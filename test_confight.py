...
class TestParse:
    ...
    def test_it_loads_strings_as_unicode(self, name, examples):
        ...
        assert_that(config, has_entry("section", has_entry("unicode", is_("💩"))))
...
class TestLoadPaths:
    ...
    def test_it_should_filter_extensions(self, examples):
        ...
        config = load_paths([str(examples.tmpdir)], extension="toml", force_extension=True)
...
class TestLoadApp(LoadAppBehaviour):
    ...
    def test_it_should_load_extra_paths(self):
        ...
        assert_that(
            self.loaded_paths(config),
            contains_exactly("/etc/myapp/config.toml", "/etc/myapp/conf.d", "/extra/path"),
        )
...
class TestCli:
    ...
    def run(self, args):
        return subprocess.run(
            [self.bin] + list(args), stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
